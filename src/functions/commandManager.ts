...
async function checkIfCommandCanBeUsed(msg: Message, commandID: string) {
	...
	if (debugLog) {
		console.log(chalk`{bgMagenta Can the user run the command?} ${commandCanBeRan}`)
	}
...
function getAllCommandIDs() {
	...
	client.commandHandler.modules.forEach((command) => {
		if (command.category.id.toLowerCase().includes('owner')) {
			return
		}
		if (command.category.id.toLowerCase().includes('testing')) {
			return
		}
		if (command.id == 'templateCommand') {
			return
		}
