...
export default class tags extends BotCommand {
	...
	async exec(message: Message, args: any) {
		...
		const addTriggers = ['add', 'create', 'make']
		const editTriggers = ['edit', 'update']
		const removeTriggers = ['remove', 'delete']

		addTriggers.forEach((thing) => {
			if (args.action == thing) {
				//check if tag name is one of the above triggers
				if (addTriggers.includes(args.tagName)) {
					return message.reply(`You can't create a tag with that name!`)
				}
				if (editTriggers.includes(args.tagName)) {
					return message.reply(`You can't create a tag with that name!`)
				}
				if (removeTriggers.includes(args.tagName)) {
					return message.reply(`You can't create a tag with that name!`)
				}

				if (!args.tagContent) {
					return message.reply(`You can't create a tag with no response!`)
				}
		...
		await database.readGuild(message.guild.id).then((guildDB) => {
			...
			tags.forEach((tag: any) => {
				if (message.content == `${this.prefix}tag ${tag.name}` && message.author.bot == false) {
